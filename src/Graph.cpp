 ...
 * CLASS VERTEX
 */
Vertex::Vertex(long id, double lon, double lat) :
		longitude(lon), latitude(lat), visited(false), processing(false), indegree(
				0), dist(0), inQueue(false) {
	this->id = id;
	path = NULL;
}
 ...
 */
int Edge::edgesID = 0;
Edge::Edge(Vertex *d, Road * road, double w) :
		dest(d), weight(w) {
	edgesID++;
	this->id = edgesID;
	this->road = road;
